...
as $$
  select exists(select 1 from public.profiles where id = p_uid and is_admin = true and is_active = true);
$$;
...
set search_path = public
as $$
  update public.profiles set last_login_at = now() where id = auth.uid() and is_active = true;
$$;
revoke all on function public.touch_last_login() from public;
grant execute on function public.touch_last_login() to authenticated;

-- Admin-only listing. It deliberately contains only identity/access metadata,
-- never study progress, questions, flashcards, or performance.
...
-- because creating Auth users requires a server-side secret and must never be
-- done with the publishable key in the APK.
